...
const GrowthRateVisualizer = () => {
  ...
  const renderMagnified = (s: string) => (
    <g>
      ...
      <clipPath id={`magnifier-clip${s}`}>
        <circle cx={MX} cy={MY} r={MR - 2} />
      </clipPath>
      <g clipPath={`url(#magnifier-clip${s})`}>
    ...
    <div className="flex flex-col py-4 select-none">
      ...
      <div className="sm:hidden mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium text-foreground">Specific Growth Rate μ₀ (/day)</span>
          <span className="text-sm font-mono font-bold" style={{ color: 'hsl(var(--accent-science))' }}>{mu.toFixed(2)} /day</span>
        </div>
        <Slider
          min={0}
          max={4.0}
        ...
        <div className="flex justify-between mt-0.5">
          <span className="text-[10px] font-mono text-muted-foreground">0 Slow</span>
          <span className="text-[10px] font-mono text-muted-foreground">4.0 Fast</span>
        </div>
      </div>
