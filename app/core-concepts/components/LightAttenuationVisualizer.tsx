...
const LightAttenuationVisualizer = () => {
    ...
    const renderDefs = (s: string) => (
        <defs>
            {/* Light attenuation gradient — fades left→right */}
            <linearGradient id={`flask-light-atten${s}`} x1="0" y1="0" x2="1" y2="0">
    ...
    const renderCultureAndChart = (s: string) => (
        <>
            ...
            <g transform="translate(0, 50)">
                ...
                {/* Culture fill */}
                <g clipPath={`url(#beaker-clip-atten${s})`}>
                    <path
                    ...
                    <rect x="40" y="40" width="120" height="180" fill={`url(#flask-light-atten${s})`} />
                </g>
                ...
                {/* Depth arrow */}
                <line x1="55" y1="228" x2="145" y2="228" stroke="var(--muted-foreground)" strokeWidth="1" markerEnd={`url(#arrowhead-atten${s})`} />
            ...
            <g>
                ...
                {/* Fill under curve */}
                <path d={genFillPath()} fill={`url(#chart-fill-atten${s})`} />
    ...
    return (
        <div className="flex flex-col py-4 select-none">
            ...
            <div className="space-y-2 mb-3 sm:hidden">
                <div>
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-mono font-bold" style={{ color: lightColor }}>{I0}</span>
                    </div>
                    <Slider min={0} max={1000} step={20} value={i0State} onValueChange={setI0State}
                ...
                <div>
                    <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-mono font-bold" style={{ color: densityColor }}>X <span className="font-normal text-muted-foreground">(g/L)</span></span>
                        <span className="text-xs font-mono font-bold" style={{ color: densityColor }}>{X.toFixed(1)}</span>
                    </div>
                    <Slider min={0.1} max={10} step={0.1} value={densityState} onValueChange={setDensityState}
                        className="w-full [&_span:first-child]:!bg-border [&_span_span]:!bg-[rgb(40,160,100)] [&_span[role=slider]]:!border-[rgb(40,160,100)] [&_span[role=slider]]:!bg-background" />
                </div>
                <div>
                    <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-mono font-bold" style={{ color: kaColor }}>ε <span className="font-normal text-muted-foreground">(m²/kg)</span></span>
                        <span className="text-xs font-mono font-bold" style={{ color: kaColor }}>{Ka}</span>
                    </div>
                    <Slider min={100} max={500} step={10} value={kaState} onValueChange={setKaState}
                        className="w-full [&_span:first-child]:!bg-border [&_span_span]:!bg-[rgb(160,90,200)] [&_span[role=slider]]:!border-[rgb(160,90,200)] [&_span[role=slider]]:!bg-background" />
                </div>
            </div>
            ...
            {/* Mobile: culture+chart above, equations below */}
            <svg viewBox="0 40 640 280" className="w-full sm:hidden mb-2">
                {renderDefs('-m')}
                {renderCultureAndChart('-m')}
            </svg>
            <svg viewBox="630 50 415 240" className="w-full sm:hidden">
                {renderEquations()}
            </svg>

            {/* Desktop: original single SVG */}
            <svg viewBox="0 0 1045 320" className="hidden sm:block w-full sm:min-w-[900px]" aria-label="Light attenuation visualization">
                {renderDefs('')}
                {renderCultureAndChart('')}
                {renderEquations()}
            </svg>
            </div>
        </div>
