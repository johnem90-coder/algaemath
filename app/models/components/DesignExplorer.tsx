...
export default function DesignExplorer() {
  ...
  const envelope = useMemo<Envelope | null>(() => {
    ...
    for (const d of depths) {
      const cfg = { ...DEFAULT_CONFIG, depth: d / 1000, harvest_mode: "none" as const };
  ...
  // Slider thumb position tracking
  ...
  const hoodTooltipLabel = (label: number) => {
    const day = Math.floor(label) + 1;
    const hour = Math.round((label % 1) * 24);
            ...
            >
              ...
              <Tooltip
                formatter={(value: number, name: string) =>
                  name === "density" ? [value.toFixed(3), "Density"] : [null, null]
                }
        ...
        <div className="w-[320px]">
          ...
          <ResponsiveContainer width="100%" height={280}>
            ...
            >
              ...
              <Tooltip
                formatter={(value: number, name: string) =>
                  name === "totalMass" ? [value.toFixed(1), "Total Mass"] : [null, null]
                }
                labelFormatter={(label: number) => {
                  const day = Math.floor(label) + 1;
                  const hour = Math.round((label % 1) * 24);
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    if (name === "lightFactor") return [value.toFixed(3), "fL"];
                    if (name === "parAvgCulture") return [value.toFixed(1), "PAR avg (µmol/m²/s)"];
                    return [null, null];
                  }}
                  labelFormatter={hoodTooltipLabel}
                />
                <Line
                  dataKey="lightFactor"
                  stroke={C_LIGHT}
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    if (name === "tempFactor") return [value.toFixed(3), "fT"];
                    if (name === "pondTemp") return [`${value.toFixed(1)}°C`, "Pond Temp"];
                    return [null, null];
                  }}
                  labelFormatter={hoodTooltipLabel}
                />
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    ...
                    if (name === "avgProductivity") return [value.toFixed(1), "Daily Avg"];
                    return [null, null];
                  }}
                  labelFormatter={hoodTooltipLabel}
                />
